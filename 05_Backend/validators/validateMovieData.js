...
function validateMovieData(data) {
  ...
  if (typeof name !== "string" || name.trim() === "" || /[^\w\s]/.test(name)) {
    return {
      valid: false,
      error: "Name must be a non-empty string without special characters.",
    };
  }
  ...
  if (typeof description !== "string" || !/^[\w\s.,!?-]+$/.test(description)) {
    return {
      valid: false,
      error:
        "Description must be a string containing only alphanumeric characters, spaces, and the following punctuation marks: . , ! ? -",
    };
  }
  ...
  if (
    ...
    !/^Screen\s\d+$/.test(screen) ||
    parseInt(screen.split(" ")[1]) < 1 ||
    parseInt(screen.split(" ")[1]) > 20
  ) {
    return {
      valid: false,
      error:
        "Screen must be in the format 'Screen [Number]' and the screen number must be within the range of 1 to 20.",
    };
  }
  ...
  if (
    ...
    language.trim() === "" ||
    !/^[a-zA-Z\s]*$/.test(language)
  ) {
    return {
      valid: false,
      error:
        "Language must be a non-empty string without numbers or special characters.",
    };
  }
  ...
  if (typeof ratings !== "number" || ratings < 0 || ratings > 10) {
    return {
      valid: false,
      error: "Ratings must be a number between 0 and 10.",
    };
  }
  ...
  if (
    ...
    return {
      valid: false,
      error:
        "Available seats must be a non-negative number and cannot exceed 1200.",
    };
  }

  // Validate PG rating
  if (
    typeof pgRating !== "string" ||
    pgRating.trim() === "" ||
    !["G", "PG", "PG-13", "R", "NC-17"].includes(pgRating)
  ) {
    return {
      valid: false,
      error:
        "PG Rating must be a non-empty string and must be one of the following values: 'G', 'PG', 'PG-13', 'R', 'NC-17'.",
    };
  }
